...
def get_expected_mdx_names(notebook_dirs: list[Path], exclude_prefix: str) -> set[str]:
    ...
    Returns filenames without extension, using dash-separated format
    (matching the conversion logic in notebook_to_mdx.py).
    """
    expected = set()
    ...
    for notebook_dir in notebook_dirs:
        ...
        for notebook in notebook_dir.rglob("*.ipynb"):
            ...
            # (underscores to dashes, matching notebook_to_mdx.py:402)
            mdx_name = notebook.stem.replace("_", "-")
...
def cleanup_orphaned_files(
    ...
    for name in orphaned_names:
        ...
        # The slug is the lowercase version of the MDX name
        slug = name.lower()
