...
- Use WherobotsDB (Sedona DataFrames and Spatial SQL) to read, write, analyze, and prepare spatial data for display. Don't use GeoPandas or DuckDB: the `forbid-geopandas-duckdb` pre-commit hook (also run in CI) fails on `import geopandas` or `import duckdb` and on `rasterflow_remote`'s `mosaic_index_gdf` accessor. Read GeoParquet outputs with `sedona.read.format("geoparquet")` and pull coordinates out with `ST_X`/`ST_Y` for `matplotlib` plots.
